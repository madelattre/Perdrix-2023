...
SAEM <- function(niter,nburnin,M,data,paraminit,paramfix,delta){
  ...
  for (k in (nburnin+1):niter){
    ...
    sigmaRes[k+1] <- alpha*sqrt(resa$minimum)
  ...
  for (i in 1:N){
    for (j in 1:nbmcmc){
      ...
      sdi <- sqrt(VarR(exp(psil), exp(psiH), seq.y, delta)) 
  ...
  BIC <- -2*ll[length(ll)] + 4*log(N) 
  
  return(list(mul=mul,muH=muH,sigma2H=sigma2H,sigma2l=sigma2l,ll=ll,BIC=BIC))
}
