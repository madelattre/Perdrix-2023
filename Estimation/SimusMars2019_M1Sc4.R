...
delta <- 1
seq.y <- c(1.1,1.2,1.3,1.5,2,3,5,10,15,25)
...
for (N in c(50,100)){
  ...
  nameFile <- paste('resM1sc4bruit_N',N,'.Rdata',sep="")
  ...
  for (k in 1:nbsimus){
    ...
    while(i<10){
      ...
      paraminit[[k]] <- list(mul=runif(1,mu_lambda-0.5,mu_lambda+0.5),
                             ...
                             sigma2l=runif(1,2*sd2_lambda,3*sd2_lambda),
                             sigma2H=runif(1,2*sd2_ch,3*sd2_ch),
                             sigmaRes=runif(1,1.5*sigma,3*sigma))
      
      
      estim[[k]] <- try(SAEM(niter,nburnin,M,perdrix,paraminit[[k]],paramfix))
      
      if(class(estim[[k]]) != "try-error"){i<-11}#Pour pas changer d'indice
...
for (N in c(20,50,100)){
  ...
  nameFile <- paste('resM1sc4bruitmeca_N',N,'.Rdata',sep="")
...
for (N in c(20,50,100)){
  ...
  nameFile <- paste('resM1sc3meca_N',N,'.Rdata',sep="")
