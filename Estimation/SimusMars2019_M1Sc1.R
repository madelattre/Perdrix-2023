...
for (N in c(100)){
  #20,50,100
  #nameFile <- paste('resM1sc1bruitAvril_N',N,'.Rdata',sep="")
  ...
  for (k in 1:nbsimus){
    ...
    paraminit[[k]] <- list(mul=runif(1,mu_lambda-0.5,mu_lambda+0.5),
                      ...
                      sigmaRes=runif(1,0.8*sigma,2*sigma))
...
sdFim <- matrix(0,nbsimus,5)
biais <- c()
param <- c()
for (k in 1:nbsimus){
  biais <- c(biais,estim[[k]]$mul[length(estim[[k]]$mul)])
  param <- c(param,'muL')
  ...
  param <- c(param,'sd2H')
  biais <- c(biais,estim[[k]]$sigmaRes[length(estim[[k]]$sigmaRes)]^2-sigma^2)
  param <- c(param,'sigma')
  sdFim[k,] <- sqrt(diag(solve(estim[[k]]$vn*N)))
}
...
ind <- which(param=="muH")
sd(biais[ind])
ind <- which(param=="sd2L")
sd(biais[ind])
ind <- which(param=="sd2H")
sd(biais[ind])
ind <- which(param=="sigma")
sd(biais[ind])

# ICinf <- c(probest,m1est,m2est) - 1.28*sqrt(diag(solve(vntheta)))
# ICsup <- c(probest,m1est,m2est) + 1.28*sqrt(diag(solve(vntheta)))
# 
# if ((prob>=ICinf[1])&(prob<=ICsup[1])){recouvprob <- recouvprob + 1}
# if ((m1>=ICinf[2])&(m1<=ICsup[2])){recouvm1 <- recouvm1 + 1}
# if ((m2>=ICinf[3])&(m2<=ICsup[3])){recouvm2 <- recouvm2 + 1}
# 
# vn[[j]] <- vntheta
# }
...
for (N in c(100)){
  ...
  #nameFile <- paste('resM1sc1bruitmeca_N',N,'.Rdata',sep="")
...
for (N in c(20,50,100)){
  ...
  #nameFile <- paste('resM1sc1meca_N',N,'.Rdata',sep="")
  ...
  for (k in 1:nbsimus){
    ...
    estim[[k]] <- try(SAEM(niter,nburnin,M,perdrix,paraminit[[k]],paramfix,delta))
...
p20 <- ggplot(res20, aes(x=param,y=biais, fill=param)) + geom_boxplot() +
  theme(plot.title=element_text(face="bold"),axis.text.x=element_text(size=14)) + xlab("") + ylab("mu") + 
  ggtitle("N=20") + ylim(-0.15,0.15)

load('resM1sc1meca_N50.Rdata')
...
# 
# for (k in 1:nbsimus){
#   mul <- c(mul, estim[[k]]$mul[length(estim[[k]]$mul)])
#   sigma2l <- c(sigma2l, estim[[k]]$sigma2l[length(estim[[k]]$sigma2l)])
#   muH <- c(muH, estim[[k]]$muH[length(estim[[k]]$muH)])
#   sigma2H <- c(sigma2H, estim[[k]]$sigma2H[length(estim[[k]]$sigma2H)])
#   sigmaRes <- c(sigmaRes, estim[[k]]$sigmaRes[length(estim[[k]]$sigmaRes)])
# }
# 
# mean(mul)
# sd(mul)
